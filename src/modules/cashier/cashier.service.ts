...
export async function commitTransaction(
  ...
  if (items.length === 0) {
    throw new CashierError(
      "Keranjang kosong — tidak ada transaksi yang dilakukan",
    );
  }
  ...
  try {
    ...
    const [variantRows, productRows] = await Promise.all([
      hasVariantItems
        ? preloadedVariants
          ? Promise.resolve(
              preloadedVariants as unknown as Record<string, unknown>[],
            )
      ...
      hasProductItems
        ? preloadedProducts
          ? Promise.resolve(
              preloadedProducts as unknown as Record<string, unknown>[],
            )
  ...
  try {
    ...
    await Promise.all([
      variantUpdates.length > 0
        ? getRepos().variants.batchUpdate(variantUpdates)
        : Promise.resolve(),
      productUpdates.length > 0
        ? getRepos().products.batchUpdate(productUpdates)
        : Promise.resolve(),
    ]);
  } catch (err) {
