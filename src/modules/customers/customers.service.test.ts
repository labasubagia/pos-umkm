...
describe("addCustomer", () => {
  it("validates phone format before appending", async () => {
    ...
    expect(mockRepos.customers.batchInsert).toHaveBeenCalledWith([
  ...
  it("throws CustomerError if phone is invalid Indonesian format", async () => {
    ...
    await expect(addCustomer("Budi Santoso", "12345")).rejects.toThrow(
      CustomerError,
    );
    await expect(addCustomer("Budi Santoso", "abc")).rejects.toThrow(
      CustomerError,
    );
  });
  ...
  it("throws CustomerError if duplicate phone already exists", async () => {
    ...
    await expect(addCustomer("Budi Santoso", "08111234567")).rejects.toThrow(
      CustomerError,
    );
  });
});
