  ...
  return rows.map((row, index) => {
    ...
    if (!Number.isInteger(row.price) || row.price <= 0) {
      return {
        row: rowNum,
        valid: false,
        error: `Harga tidak valid: "${row.price}"`,
      };
    }
    if (!Number.isInteger(row.stock) || row.stock < 0) {
      return {
        row: rowNum,
        valid: false,
        error: `Stok tidak valid: "${row.stock}"`,
      };
    }
 ...
 */
export async function bulkImportProducts(rows: ParsedProduct[]): Promise<void> {
  const results = validateImportRows(rows);
  const invalid = results.filter((r) => !r.valid);
  if (invalid.length > 0) {
    const summary = invalid.map((r) => `Baris ${r.row}: ${r.error}`).join("; ");
    throw new Error(
      `Import gagal — ada ${invalid.length} baris tidak valid: ${summary}`,
    );
  }
