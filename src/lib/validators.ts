...
export function validatePhone(phone: string): ValidationResult {
  ...
  return ok ? { valid: true } : { valid: false, error: 'Nomor telepon tidak valid (format: 08xx atau +628xx)' }
}
...
export function validatePrice(value: number): ValidationResult {
  if (!Number.isInteger(value)) return { valid: false, error: 'Harga harus bilangan bulat' }
...
export function validateQuantity(value: number): ValidationResult {
  if (!Number.isInteger(value)) return { valid: false, error: 'Jumlah harus bilangan bulat' }
