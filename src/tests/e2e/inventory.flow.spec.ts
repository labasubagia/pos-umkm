...
test.describe('Categories CRUD (T021)', () => {
  test('owner can create, rename, and delete a category', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid^="category-name-"]').filter({ hasText: 'Minuman Segar' }),
    ).toBeVisible()
    ...
    await page.getByTestId('btn-category-submit').click()
    await expect(
      page.locator('[data-testid^="category-name-"]').filter({ hasText: 'Minuman Dingin' }),
    ).toBeVisible()
    await expect(
      page.locator('[data-testid^="category-name-"]').filter({ hasText: 'Minuman Segar' }),
    ).toHaveCount(0)
...
test.describe('Products CRUD (T022)', () => {
  test('owner can add a product and it appears in the catalog product list', async ({ page }) => {
    ...
    await page.getByTestId('btn-category-submit').click()
    await expect(
      page.locator('[data-testid^="category-name-"]').filter({ hasText: 'Makanan' }),
    ).toBeVisible()
    ...
    await expect(
      page.locator('[data-testid^="product-name-"]').filter({ hasText: 'Nasi Goreng Spesial' }),
    ).toBeVisible()
    await expect(
      page.locator('[data-testid^="product-price-"]').filter({ hasText: 'Rp 15.000' }),
    ).toBeVisible()
  })

  test('product added via catalog form appears in cashier product search', async ({ page }) => {
    ...
    await page.getByTestId('btn-product-submit').click()
    await expect(
      page.locator('[data-testid^="product-name-"]').filter({ hasText: 'Mie Goreng' }),
    ).toBeVisible()
    ...
    await expect(
      page.locator('[data-testid^="product-card-"]').filter({ hasText: 'Mie Goreng' }),
    ).toBeVisible()
  })
  ...
  test('completing a sale decrements product stock', async ({ page }) => {
    ...
    await page.getByTestId('btn-tab-products').click()
    await expect(page.getByTestId(`product-stock-${prodId}`)).toHaveText('Stok: 19')
  })
})
  ...
  test('owner can run stock opname and discrepancies are logged', async ({ page }) => {
    ...
    const stockLog = await page.evaluate(
      async ({ storeId, prodId }) => {
        const db = (window as Record<string, (id: string) => { Stock_Log: { toArray: () => Promise<Record<string, unknown>[]> } }>)['__getDb'](storeId)
