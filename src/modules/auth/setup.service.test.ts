...
describe("createMainSpreadsheet", () => {
  ...
  it("throws SetupError on failure", async () => {
    vi.spyOn(adapters.driveClient, "createSpreadsheet").mockRejectedValue(
      new Error("Drive error"),
    );
    await expect(createMainSpreadsheet()).rejects.toThrow(
      "createMainSpreadsheet failed",
    );
  });
});
...
describe("createMasterSpreadsheet", () => {
  it("creates only master spreadsheet (not main)", async () => {
    ...
    expect(adapters.driveClient.createSpreadsheet).toHaveBeenCalledWith(
      "master",
      expect.anything(),
      expect.arrayContaining([...MASTER_TABS]),
    );
  ...
  it("throws SetupError on Drive API failure", async () => {
    ...
    await expect(
      createMasterSpreadsheet("Toko Z", "", "main-id-000"),
    ).rejects.toThrow("createMasterSpreadsheet failed");
  });
});
...
describe("initializeMasterSheets", () => {
  it("writes header row to all 11 required tabs", async () => {
    const makeRepoSpy = vi
      .spyOn(adapters, "makeRepo")
      .mockReturnValue(sharedMakeRepo as ReturnType<typeof adapters.makeRepo>);
  ...
  it("writes correct headers for each tab", async () => {
    ...
    const settingsIdx = makeRepoSpy.mock.calls.findIndex(
      ([, tab]) => tab === "Settings",
    );
    expect(sharedMakeRepo.writeHeaders.mock.calls[settingsIdx]).toEqual([
      ["id", "key", "value", "updated_at"],
    ]);
    const monthlyIdx = makeRepoSpy.mock.calls.findIndex(
      ([, tab]) => tab === "Monthly_Sheets",
    );
    expect(sharedMakeRepo.writeHeaders.mock.calls[monthlyIdx]).toEqual([
      ["id", "year_month", "spreadsheetId", "created_at"],
    ]);
  });
...
describe("updateStoreName", () => {
  ...
  it("updates store_name cell in main.Stores tab", async () => {
    ...
    expect(sharedMakeRepo.batchUpdateCells).toHaveBeenCalledWith([
      { rowId: "sid-1", column: "store_name", value: "Toko Baru" },
    ]);
  });
  ...
  it("throws SetupError when mainSpreadsheetId is not set", async () => {
    useAuthStore.getState().clearAuth();
    await expect(
      updateStoreName("sid-1", "New Name", "master-1"),
    ).rejects.toThrow("mainSpreadsheetId not found");
  });
  ...
  it("throws SetupError when store is not found in Stores tab", async () => {
    ...
    await expect(
      updateStoreName("sid-missing", "New Name", "master-1"),
    ).rejects.toThrow("not found in main.Stores");
  });
});
...
describe("activateStore", () => {
    ...
    sharedMakeRepo.getAll.mockResolvedValue([
      {
        ...
        spreadsheetId: "existing-monthly-id",
        id: "r1",
        created_at: "",
      },
    ]);
  ...
  it("creates monthly sheet when none exists for current month", async () => {
    ...
    const createSpy = vi
      ...
      .mockResolvedValue("new-monthly-id");
  ...
  it("reads Monthly_Sheets via makeRepo (bypasses Dexie) to avoid cross-store contamination", async () => {
    ...
    const makeRepoCall = vi
      .mocked(adapters.makeRepo)
      .mock.calls.find(([, sheetName]) => sheetName === "Monthly_Sheets");
...
describe("getCurrentMonthSheetId", () => {
  ...
  it("returns null when Monthly_Sheets tab throws (pre-setup)", async () => {
    mockRepos.monthlySheets.getAll.mockRejectedValue(
...
describe("createMonthlySheet", () => {
  it('names spreadsheet "transaction_<year>-<month>" inside the year folder', async () => {
    ...
    expect(adapters.driveClient.createSpreadsheet).toHaveBeenCalledWith(
      "transaction_2026-04",
      undefined,
      expect.arrayContaining([...MONTHLY_TABS]),
    );
    expect(mockRepos.monthlySheets.batchInsert).toHaveBeenCalledWith([
      expect.objectContaining({
        year_month: "2026-04",
  ...
  it("throws on Drive API error", async () => {
    ...
    await expect(createMonthlySheet(2026, 4)).rejects.toThrow(
      "createMonthlySheet failed",
    );
  });
});
...
describe("monthlySheetKey", () => {
  it("returns zero-padded key for single-digit months", () => {
    expect(monthlySheetKey("store-abc", 2026, 4)).toBe(
      "txSheet_store-abc_2026-04",
    );
  });

  it("returns key for double-digit months", () => {
    expect(monthlySheetKey("store-abc", 2026, 11)).toBe(
      "txSheet_store-abc_2026-11",
    );
  });
});
...
describe("runStoreSetup", () => {
  ...
  it("throws SetupError when mainSpreadsheetId is not in localStorage", async () => {
    await expect(runStoreSetup("Toko X")).rejects.toThrow(
