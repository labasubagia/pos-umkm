...
const server = setupServer(
  ...
  http.get(`${BASE}/values\\:batchGet`, () => {
    return HttpResponse.json({
      spreadsheetId: SPREADSHEET_ID,
      valueRanges: [
        { range: 'Products!A1:Z1000', majorDimension: 'ROWS', values: FAKE_VALUES },
        { range: 'Categories!A1:Z1000', majorDimension: 'ROWS', values: [['id', 'name'], ['cat-1', 'Food']] },
      ],
    })
  }),
)
    ...
    server.use(
      http.post(`${BASE}/values/:range\\:append`, async ({ request }) => {
        ...
        return HttpResponse.json({
          spreadsheetId: SPREADSHEET_ID,
          tableRange: 'Products!A1:C2',
          updates: { spreadsheetId: SPREADSHEET_ID, updatedRange: 'Products!A3', updatedRows: 1, updatedColumns: 3, updatedCells: 3 },
        })
      }),
    )
  ...
  it('sends correct range and single-cell value', async () => {
    ...
    server.use(
      http.put(`${BASE}/values/:range`, async ({ request }) => {
        capturedBody = await request.json()
        return HttpResponse.json({ spreadsheetId: SPREADSHEET_ID, updatedRange: 'Products!B2', updatedRows: 1, updatedColumns: 1, updatedCells: 1 })
      }),
    )
...
describe('sheetsBatchGet', () => {
  it('fetches multiple ranges in one call', async () => {
    const result = await sheetsBatchGet(SPREADSHEET_ID, ['Products', 'Categories'], TOKEN)
...
describe('error handling', () => {
  it('throws SheetsApiError on HTTP 403 (forbidden)', async () => {
    ...
    await expect(sheetsGet(SPREADSHEET_ID, 'Products', TOKEN)).rejects.toThrow(SheetsApiError)
  })
  ...
  it('throws SheetsApiError after max retries exceeded', async () => {
    server.use(
      http.get(`${BASE}/values/:range`, () => new HttpResponse(null, { status: 429 })),
    )
    await expect(sheetsGet(SPREADSHEET_ID, 'Products', TOKEN)).rejects.toThrow(SheetsApiError)
  })

  it('throws if token is empty or undefined', async () => {
    await expect(sheetsGet(SPREADSHEET_ID, 'Products', '')).rejects.toThrow(SheetsApiError)
  })
})
